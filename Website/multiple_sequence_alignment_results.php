...
<body>
    ...
    <?php if (empty($results)): ?>
        <p>没有找到多序列比对记录<?php if (!isset($_SESSION['user_id'])) echo "，请登录后查看账户记录"; ?></p>
    <?php else: ?>
        <table>
            <tr>
                <th>时间</th>
            ...
            <tr>
                ...
                <td>
                    <?php if (!empty($row['aligned_file'])): ?>
                        <a href="<?= htmlspecialchars($row['aligned_file']) ?>" target="_blank">多序列比对结果</a>
                    <?php else: ?>
                        <span>多序列比对结果未提供</span>
                    <?php endif; ?>
                </td>
                <td>
                    <button onclick="confirmDelete('multiple_sequence_alignment_results', <?= $row['id'] ?>)">删除</button>
                </td>
            </tr>
            <?php endforeach; ?>
        </table>
